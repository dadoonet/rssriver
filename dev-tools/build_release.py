...
def remove_documentation_snapshot(readme_file, release, branch):
    pattern = '* [%s-SNAPSHOT](https://github.com/dadoonet/rssriver/blob/%s/README.md)' % (release, branch)
    replacement = '* [%s](https://github.com/dadoonet/rssriver/blob/rssriver-%s/README.md)' % (release, release)
    def callback(line):
...
# Add in README.md file the documentation for the next version
def add_documentation_snapshot(readme_file, release, snapshot, branch):
    pattern = '* [%s](https://github.com/dadoonet/rssriver/blob/rssriver-%s/README.md)' % (release, release)
    replacement = '* [%s-SNAPSHOT](https://github.com/dadoonet/rssriver/blob/%s/README.md)' % (snapshot, branch)
    def callback(line):
        # If we find pattern, we copy the line and replace its content
        if line.find(pattern) >= 0:
...
def set_install_instructions(readme_file, artifact_name, release):
    pattern = 'bin/plugin -install fr.pilato.elasticsearch.river/%s/.+' % artifact_name
    replacement = 'bin/plugin -install fr.pilato.elasticsearch.river/%s/%s' % (artifact_name, release)
    def callback(line):
...
def tag_release(release):
    run('git tag -a rssriver-%s -m "Tag release version %s"' % (release, release))
...
def get_artifacts(artifact_id, release):
    artifact_path = ROOT_DIR + 'target/release/%s-%s.zip' % (artifact_id, release)
...
def git_push(remote, src_branch, release_version, dry_run):
    if not dry_run:
        ...
        run('git push %s rssriver-%s' % (remote, release_version)) # push the tag
    else:
...
<p>- <a href="http://twitter.com/dadoonet">David</a></p>
</body></html>
""" % {'release_version': release_version,
       'artifact_id': artifact_id,
       'artifact_name': artifact_name,
...
Release successful pending steps:
    ...
    * check if the release is there https://oss.sonatype.org/content/repositories/releases/fr/pilato/elasticsearch/river/%(artifact_id)s/%(version)s
